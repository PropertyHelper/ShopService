...
class SQLCashierRepository(SQLBaseClass, AbstractCashierRepository):
    async def save_cashier(self, cashier_create: CashierCreate) -> Cashier:
        async with self.get_session() as session:
            cashier_model = CashierModel(cid=cashier_create.cid, password_hash=cashier_create.password,
                                         account_name=cashier_create.account_name, shop_id=cashier_create.shop_id)
    ...
    async def get_password_hash(self, shop_nickname: str, account_name: str):
        async with self.get_session() as session:
            stmt = select(CashierModel.password_hash)\
                ...
                .join(ShopModel)\
                .where(ShopModel.nichname == shop_nickname)
